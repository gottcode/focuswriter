...
: QDialog(parent),
  m_document(document) {
	...
	connect(find_button, SIGNAL(clicked()), this, SLOT(findNext()));

	QPushButton* replace_button = new QPushButton(tr("Replace"), this);
	connect(replace_button, SIGNAL(clicked()), this, SLOT(replaceNext()));
